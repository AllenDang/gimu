	...
	if opt == PlatformInstallCallbacks {
		...
		win.SetMouseButtonCallback(func(w *glfw.Window, button glfw.MouseButton, action glfw.Action, mods glfw.ModifierKey) {
			...
			if action == glfw.Press {
				dt := glfw.GetTime() - lastButtonClickTime
				if dt > doubleClickLO && dt < doubleClickHI {
					isDoubleClickDown = 1
					doubleClickPos.SetX(float32(x))
					doubleClickPos.SetY(float32(y))
				}
				lastButtonClickTime = glfw.GetTime()
			} else {
				isDoubleClickDown = 0
			}
		})
...
func NkPlatformNewFrame() {
	...
	NkInputButton(ctx, ButtonDouble, int32(doubleClickPos.X()), int32(doubleClickPos.Y()), isDoubleClickDown)
...
var (
	...
	lastButtonClickTime float64
	isDoubleClickDown   int32
	doubleClickPos      Vec2
)
